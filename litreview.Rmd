...
Other interactive data visualization packages developed for R tended to focus on specific methods and techniques. For instance, `KLIMT` was developed for interactive visualization of classification and regression trees [@urbanek2001; @urbanek2002]. Similarly, packages like `tourr` [@wickham2011] and `liminal` [@lee2021; @lee2022] provided functionalities for exploring large multivariate data sets via grand tour projections [see @cook1995]. Overall, over time, within the R community, there seems to have been a trend towards more of these specialized tools, and the development of general-purpose interactive data visualization frameworks seems to have become less prevalent [although there are some notable exceptions, such as the `loon` package, @waddell2023]. Currently, it seems that R users typically encounter interactive visualizations as part of Shiny [@shiny2024] dashboards, or through R wrappers of interactive data visualization packages from the JavaScript ecosystem (see Section \@ref(web-based)).
...
Querying is useful because combines the best features of graphics and tables. Specifically, it allows the user to overcome Tukey's famous prescriptions: "graphics are for the qualitative/descriptive [...] never for the carefully quantitative (tables do that better)", and: "graphics are for comparison [...] not for access to individual amounts" [@tukey1993]. Providing the option to query individual objects gives the user the ability to seamlessly transition between the high-level analytic overview of the graphic and low-level quantitative detail of a table. This allows users to perform tasks requiring higher precision, such as identifying specific outliers or calculating exact magnitudes of differences [@unwin2006].
...
As discussed in Section \@ref(complexity-of-features), another valuable class of interactive features are those which affect the computation of the summary statistics underlying the graphic [also called "parametric interaction", @leman2013; @pike2009]. These features extend beyond simple manipulation of visual attributes, requiring that user interaction penetrates much deeper inside the data visualization pipeline. Fundamentally, these features involve the manipulation of the parameters of some underlying mathematical model.    

A basic and common example of parameter manipulation is dynamically changing histogram binwidth or anchor. Assuming a fixed binwidth $w$ and an anchor $a$, we can describe a histogram via a function $h$ that, for each observation of a continuous variable $x_i$ returns an index $j$ of the corresponding bin, such that, for an ordered list of bins breaks $b_j$, we have $x_i \in [b_{j}, b_{j + 1})$^[Technically, if there are any values $x_i < a$, we will have negative indices ($j < 0$), and if all values are significantly larger than the anchor, such that $x_i > a + w$, the indices will not start at 1. So, to implement the histogram properly, we should shift all indices by subtracting the minimum index. Finally, if the histogram binwidth is not fixed, the function becomes more complicated as well.]:

$$h(x; a, w) = \lfloor (x_i - a) / w \rfloor + 1$$

Thus, a histogram really is a kind of a mathematical model, and can in fact be seen as a crude form of density estimation [see also e.g. @bishop2006, pp. 120-122]. Manipulating histogram bins amounts to manipulating the parameters of the function $h$. Importantly, unlike features like zooming and panning, which involve manipulating visual attributes of the plot only, changing binwidth and anchor requires recomputing the underlying summary statistics. These changes can have significant downstream effects. For instance, if changing the binwidth leads to the count of cases within the tallest histogram bar increasing, we may need to update the upper y-axis limit, if we want to prevent the bars from overflowing the plotting region.   

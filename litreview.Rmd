...
This feature is usually quite simple to implement, since it involves the visual attributes of geometric objects only. In other, when we manipulate size or alpha of geometric objects in our plots, we usually do not need to worry about where the data underlying these objects came from. 
...
The same data can often by visualized in multiple ways, by employing different sets of visual encodings [@wilkinson2012]. However, some encodings may work better for answering certain questions than others. By enabling users to dynamically switch between different visual representations, we empower them to delve deeper into the data and answer a wider array of questions [@yi2007].
...
However, barplots are less effective for comparing proportions. Since the heights of the bars are different, it is difficult to compare relative proportions of segments across bars. Spineplot, on the other hand, presents a way of visualizing the same sort of data as a barplot while making it much easier to compare proportions. Specifically, in a spineplot, the heights of the bars are all normalized to 1, such that the segments show a proportion, and the absolute values are instead encoded as the bar width. The fixed height of bars makes it easy to compare the proportions the segments represent.

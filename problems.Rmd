...
While all plots inherently involve at least one partition which defines the mapping between the underlying data and the geometric objects, many plot types partition the underlying data across multiple dimensions. Specifically, multiple variables may be used to partition the data, such that each data point is mapped to the unique combination of these variables. This multidimensional partitioning is particularly prevalent in interactive data visualization, where features such as linking automatically introduce an additional partitioning variable [@wilhelm2008]. Consequently, a generalized mechanism for deriving Cartesian products of partitioning variables is required.

The idea of partition products may be best illustrated on concrete code examples. In R, the canonical data structure for creating a grouping structure (partition) is the S3 `factor` class. We can use `factor`s to partition our data and compute various aggregate summaries over the parts: 

```{r}
group <- factor(c("A", "A", "A", "B", "B", "C", "C", "C"))
income <- c(170, 150, 200, 300, 150, 100, 220, 140)
...
With multiple partitioning variables, we can compute summary statistics on finer segments of our data. While base R does not have a standalone data structure for products of multiple `factor`s^[It is not too difficult to create this data structure: see Section \@ref(product-factors).], many built-in functions create this data structure implicitly, when supplied with `list()` or formula arguments (using the `+` operator on the right-hand side of the formula to define the product): 
...
Now, clearly, products of partitions are useful because they allow us to compute summary statistics at greater level of granularity, and this is particularly useful in data visualization. For instance, here's how we could use `ggplot2` [@wickham2016] to display the aggregated data above via a stacked barplot:
...
In fact, these partition products are so useful that many data visualization systems [particularly ones based on the Grammar of Graphics, @wilkinson2012], construct them automatically, based on plot specification. For instance, we could produce the same plot without having to explicitly aggregate the data ourselves by running the following `ggplot` call:
...
It is useful to pause and analyze what is happening here. The `ggplot` function takes the variables specified inside the `aes` call and uses these to construct a grouping structure (partition), by taking their product. Importantly, it also automatically infers which variables are meant to form the grouping structure, and which are meant to be aggregated over this structure. For instance, in the example above, the `income` variable is *not* included in the partition product, since it is a continuous variable mapped to the y-axis, and this, in combination with the geometric object (`geom_bar`), marks it as the variable to be aggregated. See also the following code comment:
...
Geometric objects directly representing the data (which I will call geoms, from here on out) are of primary concern, since these are the most likely cause of rendering bottle-necks: typically, they are the most numerous class of graphical elements as well as the most likely to require re-rendering upon interaction. For instance, in scatterplots and lineplots, the number of geoms equals the number of rows. Likewise, in barplots and fluctuation diagrams, larger data set size often implies a greater number of categories, and as a consequence more objects to be rendered. Furthermore, most interactive features affect the geoms in some way, necessitating re-rendering. For instance, zooming, panning, and reordering affect the objects' position and/or size, while selection and parametric interaction affect the objects internal attributes, as well as the number of objects to render itself.

In contrast, auxiliary graphical elements are typically few in number, and require less frequent re-renders. For instance, in a typical plot with continuous axes, there are generally only 4-6 labels for each axis (as an example, in R, the `pretty` function uses `n = 5` as the default number of *desired* intervals used to find axis breaks) and a single title. With two axes, that amounts to just 10-14 text strings (axis labels plus titles) that need to be rendered, which is generally much smaller number than the number of geoms. Additionally, as was mentioned in Section \@(stacking-part-whole), with monoidal plots like stacked barplots and histograms, axis labels do not have be to be re-rendered during linked selection. They *do* need to be re-rendered during zooming and panning, however, this operation will typically be fairly cheap. Axis titles need to be re-rendered only during representation switching, such as when switching from a barplot to spineplot.

#### Rendering: A functor from summary statistics to graphics {#rendering-functor}

Importantly, just as we can think of aggregation as a functor from the category of data subsets to the category of summary statistics (see Section \@ref(aggregation-functor)), so can we interpret rendering as a functor from the category of summary statistics to the category of graphics. Specifically, if the statistics underlying the plot form a hierarchy, so should the geometric objects representing them, and the mapping from one to the other should preserve the inherent structure. 

Practically, this means that the geoms we use to represent the data should also behave like monoids: they should have a notion of an empty element and a binary associative operations that combines two elements together. For most simple geometric shapes, this is easily satisfied. For instance, bars, or more generally rectangles, can be combined by stacking or layering. Note that this does not necessarily require summing the bar heights. For instance, we can combine two bars by "cutting off" the extra height of the taller bar and pasting the resulting segment on top of the shorter bar (and the operation will be unital and associative, provided that the bars are sorted in increasing order by height).

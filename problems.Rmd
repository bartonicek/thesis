...
#### Some statistics are not stackable

The visualization in \@ref(fig:grammar-barplot-means) is problematic because the bar heights lack a meaningful statistical interpretation. The sum of group means is not something that most consumers of visualizations would know how to interpret or care about. Whereas before, in Figure \@ref(fig:grammar-barplot-counts), the heights of the stacked bars represented valid overall counts - the number of smokers and non-smokers within a given age category combined - however, this is no longer the case  in Figure \@ref(fig:grammar-barplot-means). The sum of the group means is different from the mean of the combined cases, and so is the mean of the group means.
...
Before, I had formalized the hierarchy of data subsets as a preorder. Now, we need to take a further step towards abstraction by describing preorders within the framework of category theory, specifically as categories.

Although often perceived as complex, the definition of a [category](#categories) is actually quite straightforward. In simple terms, a category $\mathcal{C}$ is a collection of objects (denoted as $\text{Ob}(\mathcal{C})$), connected by sets of arrows (morphisms), with four fundamental properties: 

\newcommand\then{⨾}

- Every object $c \in \text{Ob}(\mathcal{C})$ has an arrow $\text{id}_c$ pointing to itself (called the identity morphism)
...
One example of a category where the objects are elements in a set is a preorder. Specifically, given a preorder on a set $S$, define the objects $c \in \text{Ob}(\mathcal{C})$ as the elements in $s \in S$, and, for any two objects $a$, $b$, define at most one morphism $a \to b$ if $a \leq b$ (and no morphism if $a \not \leq b$). Then, the two properties of preorders just fall out of the definition of a category:
...
While the benefits of this abstract reformulation of preorders might not be immediately apparent, I encourage the reader to bear with me. In the next section, I will show the advantages of thinking about our data in this way.

#### Structure preserving maps: Functors

A fundamental concept in category theory is that of a structure-preserving map or a [functor](#functors). As the name suggests, a functor is a mapping from one category to another, such that the properties of the former category are preserved or reflected in its embedding in the latter [this can also be thought of as drawing a diagram of the former category inside the latter, @fong2019]. 
...
Further, this mapping is subject to two properties:

- Identities are preserved: $F(\text{id}_c) = \text{id}_{F(c)}$
- Composition is too: $F(f \then g) = F(f) \then F(g)$

The first property simply states that objects cannot be separated from their identities. The second property is more interesting, since it tells us that all compositions (chains of arrows) must be preserved. Other than that, we are free to map the objects and arrows as we wish. We can map multiple objects in $\text{Ob}(\mathcal{C})$ to a single object in $\text{Ob}(\mathcal{D})$, "squish" a morphism (or a chain of morphisms) in $\mathcal{C}$ by mapping it to an identity morphism in $\mathcal{D}$, or "stretch" a morphism in $\mathcal{C}$ by mapping it to a composite morphism in $\mathcal{D}$. However, we cannot "tear" or "rip" any chain of morphisms in $\mathcal{C}$ into multiple parts in $\mathcal{D}$.  

#### Aggregation: A functor from a preorder of data subsets to a preorder of statistics

As mentioned before, we start with a preorder of data subsets ordered by inclusion, which can be re-interpreted as a category. We would like to summarize these subsets in a way that preserves the preorder's structure. Is there a way to describe this? Indeed there is: a functor.

But before we go on to do that, we need to do one more trick. We can recast the set inclusion relations in Figure \@ref(fig:barplot-preorder) as unions. Specifically, instead of the arrows simply denoting the fact that, for example, segment 1 is a subset of bar 1, we can be explicit about the fact that we can get to bar 1 by taking the union of segment 1, and *all the other segments within the bar*: 
...
Now, an important fact about set union is that it is associative. Suppose we have three disjoint subsets of our data $S_1$, $S_2$, and $S_3$. Since set union is an associative operation, we know that we can take the union of the three subsets in any order and always end up with the same result. For example, we could first take the union of sets $S_1$ and $S_2$, and then union that with $S_3$ (i.e. $B_1 = (S_1 \cup S_2) \cup S_3$), or we could first union $S_2$ and $S_3$ and then union that with $S_1$ (i.e. $B_1 = S_1 \cup (S_2 \cup S_3)$). This fact can be described by the following commutative diagram:    
...
This fact is also true of taking the union of the subsets of our data. In other words, *the subset corresponding to a bar will be the same, no matter in which order we take the union of subsets corrresponding to segments.* 

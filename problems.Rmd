...
For many types of plots, the strategy of taking products of factors to form a single "flat" partition of the data works reasonably well. However, for other types of plots, this simple model breaks. For example, returning back to the example of the barplot from the section above, we may want to represent the same underlying data in a different way, as a spineplot:

```{r barplot-spineplot}
#| echo: false
#| fig-cap: "The same underlying data that can be represented by a barplot (left) can be represented by a spineplot (right)."

knitr::include_graphics("./figures/barplot-spineplot.png")
```

In a spineplot, the underlying summary statistics (sum, in this case) gets mapped to two aesthetics: the y-axis position/height and the bar width. Specifically, the y-axis position represents proportion of the summary (sum), normalized within each bar, and bar width represents the absolute value of the summary. This type of representation is very useful for comparing proportions. Notice that the spineplot in Figure \@ref(fig:barplot-spineplot), makes it much easier to see that the proportions of the red cases are same across the B and C groups.

...
- Map $A \to C$ (via $f \then g$) and then apply the functor ($F(\text{id}_C)$)
- Map $A \to B$ (via $f$), apply the functor ($F(\text{id}_B)$), and then map $F(B) \to F(C)$ (via $F(g)$) 
- Apply the functor immediately ($F(\text{id}_A)$) and then map $F(A) \to F(C)$ (via $F(f \then g) = F(f) \then F(g)$)
...
As we have established before, when visualizing data, we may start with a preorder of data subsets ordered by sibling subset union, as in Figure \@ref(fig:barplot-preorder2) (reproduced below for reminder). We would like to translate these data subsets into summary statistics, in a way that preserves the inherent structure in the data subset preorder. As we will see, the way to do this is via a functor.

```{r barplot-preorder22}
#| echo: false
#| fig-cap: "A diagram of a barplot/spineplot represented as a preorder of data subsets corresponding to a set union, reproduced."
knitr::include_graphics("./figures/barplot-preorder2.png") 
```

But first, let's discuss the summary statistics. Since we want to summarize each data subset, then, it is given that we should end up with an equal number of summaries. For instance, if $D = \{ D_D, D_{B_1}, \ldots D_{B_n}, D_{S_1}, \ldots, D_{S_k} \}$ is the set of data subsets corresponding to the whole data set, bars, and segments, respectively, and $S$ is the set of summaries, then $\lvert D \lvert = \lvert S \lvert$. Further, since $D$ is actually part of a preorder, then, intuitively, $S$ should be a part of a preorder as well. 

What should be the order relation between the summaries in $S$? In $(D, \cup)$, the order relation is given by set union with the sibling subsets. In the preorder of summaries, we want the order relation to be some operation that "behaves like set union", in a way that it represents the operation of *combining sibling summaries*. We can label it with the symbol $\otimes$, such that the resulting preorder is $(S, \otimes)$:

```{r}
#| echo: false
#| fig-cap: "A diagram of barplot/spineplot summary statistic preorder, ordered by the combination operator $\\otimes$." 
knitr::include_graphics("./figures/barplot-preorder4.png") 
```

As before, each $\otimes$ in the diagram above corresponds to the operation of combining a summary with the other sibling summaries, such that, for example, the label for the arrow $S_{S_1} \to S_{B_1}$ should be $- \otimes S_{S_1} \otimes S_{S_2}$. In words, "a segment subset combines into a bar subset".

Now comes the key point. Suppose that we have some summary operation $F: D \to S$ which maps data subsets to summaries. To preserve the structure of $(D, \cup)$, the operation should be functorial, such that, when joining some subset with two (disjoint) subsets $D_{i}$ and $D_{j}$ and summarizing:

$$F(- \cup D_{i} \then - \cup D_{j}) = F(- \cup D_i) \then F(- \cup D_j)$$

The expression above looks somewhat awkward, however, it can be simplified by nothing that:
...
- We can omit the composition operator $\then$. This is due to the fact that $\cup$ and $\otimes$ are binary operators which are also associative (by definition of composition in categories). As an example, if $f(x) = - \otimes 2 = x + 2$, then we can write $[f \then f](x) = f(f(x)) = ((x + 2) + 2) = x + 2 + 2$)

This leads to the following simplification:

$$F(- \cup D_i \cup D_j) = - \otimes F(D_i) \otimes F(D_j)$$

Finally, without loss of generality, we can choose the empty set $\varnothing$ as the starting subset ($-$), and then the equation reduces to:
...
In other words, *it should not matter whether we first take the union of the underlying data and then summarize the resulting superset, or first summarize the subsets and then combine them via the operation $\otimes$ (the summary should distribute across set union)*.
...
$$F(\text{id}_{D_i}) = F(D_i \cup \varnothing) = F(D_i) \otimes F(\varnothing) = F(D_i) = \text{id}_{F(D_i)}$$

This means that the summary operation $F$ must be defined for the empty set, and should return some "neutral" value $F(\varnothing)$, which, when combined with any other summary, just returns the same summary back. 

Second, as per Equation \@ref(eq:summary-functor-composition), the summary should distribute across set union:

$$F(D_i \cup D_j) = F(D_i) \otimes F(D_j)$$

This distributive property should hold over *all* disjoint subsets of the data as these correspond to all possible arrows we can draw in the set union diagram of the data subset preorder. Further, we can imagine any set $D_i$ to be the union of the smallest possible (non-empty) disjoint subsets: individual data points (rows) $d_j \in D_i$. Thus, the summary operation should distribute over data points:

$$F(d_1 \cup d_2 \cup \ldots \cup d_n) = F(d_1) \otimes F(d_2) \otimes \ldots \otimes F(d_n)$$

Thus, in a certain way, $F$ *really is* the binary combination operator $\otimes$. We can write:

$$F(d_1 \cup d_2 \cup \ldots d_k) = d_1 \otimes d_2 \otimes \ldots d_n$$

Just to drive the point home, we can also imagine going the other way around and combining the summaries of two data subsets $D_i$ and $D_j$. Then:
...
Again, this shows that combining summaries leads to a valid summary of the underlying subset union, which is precisely the desired property we set out to find.

#### Stackable summaries: Monoids

I have argued that, for plots to be well-behaved under certain kinds of interactions such as linked selection, the summary statistics we display should "behave like set union". Specifically, they should be a functor from the preorder of data subsets. This has lead to two key insights: the summary statistic should be based on some binary associative operation $\otimes$, and the summarized values should be equipped with some "neutral" value that is the result of summarizing an empty set. It turns out there is a well-known algebraic structure with these exact properties: monoids. 
...
While most summary statistics we care about in data visualization are numeric, this is still an interesting fact to ponder.

Further, statements about statistics being monoids translate fairly well to code. Specifically, if we take set union to be vector/array concatenation (the `c()` function in R), and provided we have some representative data vectors, we can test the properties of an arbitrary summary function. For example, here's how we can test whether `sum` and `mean` are associative:
...
sum(c(c(x, y), z))      # F(f;g)
sum(c(sum(c(x, y)), z)) # F(f);F(g)

mean(c(c(x, y), z))       # F(f;g)
mean(c(mean(c(x, y)), z)) # F(f);F(g)
```

We could even create a wrapper function to test whether a given function with a specific neutral element forms a monoid:
...
vector_length <- function(x) sqrt(sum(x^2)) # Alternatively, euclidean distance

is_monoid(sum, 0, x, y, z)
is_monoid(max, -Inf, x, y, z)
is_monoid(prod, 1, x, y, z)
is_monoid(string_concat, "", "a", "b", "c")
is_monoid(vector_length, 0, x, y, z)
is_monoid(mean, 0, x, y, z)
is_monoid(median, 0, x, y, z)
is_monoid(sd, 0, x, y, z)

# Technically, all of the functions should be binary and take 2 scalar arguments,
# i.e. string_concat and vector_length should be defined as function(x, y),
# however, I've defined them here as unary vector functions to be consistent with
# mean, median, and sd.
```

Of course, technically, the properties of associativity and unitality should hold for *all* possible values, thus testing that a summary function passes the `is_monoid` test for any three specific data vectors is not sufficient to call it a monoid. However, a single counter-example does disprove the summary function being monoidal, as is the case, for example, with `mean`, `median`, and `sd` above.

...
```{r}
#| results: "hold"
is_functorial <- function(fn, x, y, z) {
  fn(c(c(x, y), z)) == fn(c(fn(c(x, y)), z))
}

is_functorial(sum, x, y, z)
is_functorial(max, x, y, z)
is_functorial(prod, x, y, z)
is_functorial(function(x) sqrt(sum(x^2)), x, y, z) # Euclidean distance
is_functorial(mean, x, y, z)
is_functorial(sd, x, y, z)
```
...
```{r}
#| echo: false
#| fig-cap: "A diagram of barplot/spineplot summary statistic preorder, ordered by the combine operator $\\otimes$." 
knitr::include_graphics("./figures/barplot-preorder4.png") 
```

As before, each $\otimes$ in the diagram above corresponds to the operation of combining a summary with the other sibling summaries, such that, for example, the label for the arrow $S_{S_1} \to S_{B_1}$ should be $- \otimes S_{S_1} \otimes S_{S_2}$.
...
The expression above looks somewhat hairy, however, it can be simplified by nothing that:

- $F(- \cup D_i ) = - \otimes F(D_i)$ (by definition of our functor: lifting a union operation to the space of summaries amounts to combining summaries)
- Since $\cup$ and $\otimes$ are binary operators, we can omit the composition operator $\then$

Leading to:

$$F(- \cup D_i \cup D_j) = - \otimes F(D_i) \otimes F(D_j)$$

Without loss of generality, by choosing the empty set $\varnothing$ as the starting subset ($-$), the equation reduces to:
...
In other words, *it should not matter whether we first take the union of the underlying data and then summarize the resulting superset, or first summarize the subsets and then combine them via the operation $\otimes$*.
...
This distributive property should hold over *all* disjoint subsets of the data as these correspond to all possible arrows we can draw in the set union diagram of the data subset preorder. The smallest (non-empty) disjoint subsets are individual data points $d_i \in D$. Thus, the summary operation needs to distribute over data points:

$$F(d_1 \cup d_2 \cup \ldots \cup d_k) = F(d_1) \otimes F(d_2) \otimes \ldots \otimes F(d_k)$$

Thus, in some way, $F$ *really is* the combination operator $\otimes$. We can write:

$$F(d_1 \cup d_2 \cup \ldots d_k) = d_1 \otimes d_2 \otimes \ldots d_k$$

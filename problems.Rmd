...
Although barplot and spineplot represent the same underlying data, turning one into the other is not always easy. Specifically, while many grammar-based visualization systems offer a simple declarative syntax for defining barplots, they lack such simple syntax for spineplots. For instance, to draw a spineplot in `ggplot2`, we first need to do a substantial amount of data wrangling (creating the plot in the right panel of Figure \@ref(fig:barplot-spineplot) took over 10 lines of code, using standard `dplyr` syntax). The same applies to other "normalized" types of plots, such as spinograms [see e.g. @theus2002].
...
However, despite its advantages, the grammar-based model has one fundamental flaw: graphics and statistics are not truly independent. Instead, the visual representation of data must be congruent with its mathematical properties. This constraint, while present even in static visualizations, becomes especially critical in interactive contexts. Let's illustrate this point with an example of real-world data.
...
#### Advantages of stacking: Part-whole relationship
...
#| echo: false
#| fig-cap: "Stacking has advantages over dodging (and layering) when it comes to displaying linked selection. Plots left to right show a simulated static snapshot of what happens as more cases are selected (red). In a stacked barplot (top row), the heights of the higlighted segments are always bound by the height of the whole bar, and so the outline of the figure remains constant. In contrast, in a dodged barplot, the bar segment heights are not bounded, leading to the outline of the figure fluctuating dramatically (notice the changing upper y-axis limit)."
knitr::include_graphics("./figures/stacking-vs-dodging.png") 
```

The only reference I have been able to find which addresses this issue directly is @wilhelm2008. Wilhelm outlines three linked visualization strategies: replacement, overlaying, and repetition. Within the context of the three techniques discussed above (stacking, dodging, and layering), overlaying essentially conflates both stacking and layering, repetition is equivalent to dodging, and replacement involves re-rendering the visualization completely upon selection. Wilhelm argues that replacement is a flawed because it entirely discards contextual information such as axis limits. He also notes that repetition is less common due to the necessity of re-arranging the plot upon selection. Finally, he identifies two issues with overlaying: the fact that plot parameters are inherited from the plot representing the whole data set, and the fact that parts of the original plot may become obscured by the highlighted subset. Ultimately, he appears to favor repetition over the other two methods.
...
### Stackable summaries: A sojourn into Category Theory
...
Now, imagine trying to explain the diagram above to a computer. As humans, we have internalized the fact that set union is associative (meaning that the order in which we combine sets does not matter). As such, we can naturally interpret the statement "a bar subset is a union of all segment subsets" as a single action: taking all segment subsets and "smushing" them together. However, computers do not work this way. Instead, they process instructions sequentially. For a computer, there is no such thing as a "global" union of subsets, there is only a sequence of pairwise unions. In other words, you cannot tell a computer: "take all sets and combine them together", instead, you can only tell it: "take $S_1$ and combine it with $S_2$, take the result and combine that with $S_3$, ..." and so on.

However, we can describe the fact that we want the sequential operations to be associative via a diagram. Specifically, the following commutative diagram: 
...
This is the most important point in this section, and one of the most important points in the whole thesis. The definition of a functor tells us which summary statistics can meaningfully preserve the part-whole relationships found in a hierarchical partitioning in the data, and this in turn tells us whether they will be "stackable" and behave well under linked selection. 

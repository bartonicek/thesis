...
Additionally, in some data visualization systems, certain elements of the visualization may be rendered out-of-frame entirely. For instance, in web-based interactive visualization frameworks, some elements may be represented via external DOM nodes, e.g. a query pop-up may be represented via a `div`/`table` element, independent of the `canvas`/`svg` element used to render the data. This can simplify rendering logic - for example, in the case of the query pop-up, we can offload the work of rendering the table to the browser, instead of having to implement the logic of rendering a table manually.   

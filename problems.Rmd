...
For many types of plots, the strategy of taking products to form a single "flat" partition of the data works reasonably well. However, for other types of plots, this simple model breaks. For example, returning back to the example of the barplot from the section above, we may want to represent the same underlying data in a different way:
...
p1 <- ggplot(combined_df, aes(x = group, y = value, fill = selection)) +
  geom_col(position = position_identity(), col = "white") +
  custom_theme +
  theme(plot.margin = unit(c(0, 0.5, 0, 0.5), units = "cm"))

p2 <- ggplot(df_spine2, aes(position, prop_cum, width = sum, fill = selection)) +
  geom_bar(stat = "identity", position = "identity", col = "white") +
  geom_text(data = df_spine1, aes(x = position, y = 0.9, label = group, fill = NULL),
            col = "white") +
  scale_x_continuous(breaks = c(0, df_spine2$right)) +
  custom_theme +

...
Because of these problems, during the development of `plotscape`, I decided to explore alternative strategies for handling reactivity.
...
```{ts}
type EffectFn = () => void;
const currentEffects = [] as EffectFn[];

function getCurrentEffect(): EffectFn {
  return currentEffects[currentEffects.length - 1];
}

function createSignal<T>(x: T): [() => T, (value: T) => void] {
  const listeners = new Set<EffectFn>();

  function get(): T {
    listeners.add(getCurrentEffect());
    return x;
  }
...
function createEffect(effectfn: EffectFn) {
  function call() {
    currentEffects.push(call);
    effectfn();
    currentEffects.pop();
  }
...
const [tax, setTax] = createSignal(0.15);

...
Section \@ref(problems) outlined several key concerns that arise when designing generic interactive data visualization systems. Overcoming these issues necessitated specific architectural choices, which will be detailed throughout the rest chapter. The present section provides a simplified, high-level overview, briefly introducing the system components and their relations without going into too much depth. More specific, low-level details will be discussed later, in Section \@ref(low-level-implementation) (however, links to the corresponding subsections will be provided where appropriate).

### Scene model

As discussed in Section \@ref(high-level-api), every `plotscape`/`plotscaper` figure is composed of one or more interactive plots. Thus, the central component is a [Scene] which represents the entire figure which contains the plots (see Figure \@ref(fig:scene-diagram)). To support linked selection (which affects all plots simultaneously), the scene needs to contain a specialized [Marker](marker) object. For communication with an external server (R client), a client (WebSockets) object is also included. Lastly, the Scene contains^[This could also be implemented by class inheritance, by making the Scene object extend an Observer class.] an event listener object to coordinate user input and interactions between all components. The relations between the various components are described by the following diagram:
...
### Summaries and hierarchical data
...
Specifically, if we want to render geometric objects with highlighted parts representing selection requires a tree-like structure (see e.g. Figure \@ref(fig:spineplot-tree)). In other words, our system needs components that can represent a hierarchy of connected data tables with the following shape: 
...
To give a concrete example, to draw a spineplot, a simple table of summaries computed over a "flat" product of two variables like this is not enough:

```{r}
aggregate(mpg ~ cyl + am, FUN = sum, data = mtcars)
```

To compute the heights of the normalized bar segments, we require parent-level data. One way to add this information is via a left join:

```{r}

df_parent <- aggregate(mpg ~ cyl, FUN = sum, data = mtcars)
df_child <- aggregate(mpg ~ cyl + am, FUN = sum, data = mtcars)

df_child |>
  merge(df_parent,by = "cyl", all.x = TRUE) |> # Join parent-level data
...
There are several important things to note here. First, as discussed in Section \@ref(limits-of-flat-products), the example above shows that some plots fundamentally involve multiple aggregated data sets, computed at different levels of granularity. Crucially, this means that the aggregation scheme remains consistent across these data sets, and only the partitioning (the right-hand side of the formula, i.e. `~ cyl` and `~ cyl + am`) changes. For example, normalizing spineplot segment values requires calculating the same summary (`sum`) at both parent and child levels. Further, while in the example above, we compute only a single summary statistic (`sum`), we may want to compute multiple, e.g. also `max`, `min`, and `median`. Thus, a better way to describe this operation may be to specify the summary statistics we want to compute and the partitions we want to compute them over as separate arguments to a single function, e.g. something like the following pseudocode:

```{r}
#| eval: false
compute(
  summaries = list(stat1 = sum(mpg), stat2 = max(hp)), 
  partitions = c(~ cyl, ~ cyl + am),
  data = mtcars
  )
```

Second, it is also important to recognize that the relationship between the aggregated data sets is truly hierarchical, and attempts to flatten it may lead to complications. For instance, in the example above, we flatten the hierarchy via SQL-like left outer join [@codd1970]. However, I argue that this approach has several disadvantages. First, we have to represent the parent-child relationships by renaming columns (e.g. `mpg.x` and `mpg.y` in the example above). This is a flawed solution since the new column names could potentially conflict with existing variable names. Further, it needlessly duplicates information - the parent-level values have to be repeated for each corresponding level of the product (child) partition. Fundamentally, the parent and child level data sets contain all necessary information; thus, instead of "materializing" the relationships via joins, it may be more efficient to represent it implicitly via some kind of reference (e.g. pointers).

Third, another disadvantage of the simple join model is that it discards information about the function that was used to compute the summaries. For instance, in the example above, we use the `sum` function to aggregate the `mpg` variable within the factor levels, followed by `cumsum` to stack the result, and these are treated as separate operations. However, in light of the ideas discussed throughout Section \@ref(aggregation), we may note that this is really just repeated application of the $(\mathbb{R}, 0, +)$ or the summation monoid (see Section \@ref(monoids)). In other words, the operation underlying `sum` and `cumsum` is the same, the only thing that changes is how we cumulate the result. Consequently, specifying two different functions is redundant. Instead, we could simply specify that we want to use `sum` as the summary function and later indicate that we want to stack the resulting values, and the system should be able to apply the operation correctly. This requires storing metadata about the summarizing function on the result; the aggregated variable needs to "remember" how it has been computed. As laid out in Section \@ref(stacking-not-graphical), I believe this approach is warranted and necessary, since stacking is *not* simply a graphical operation - there is no such thing as stacking *a* variable, if we do not know how it's been computed.

This leads to the idea of reduced variables and reducers (which is just an alternative name for monoids). Specifically, after we aggregate a variable, we append the metadata about the reducer/monoid onto the result, like so:

```{r}
df <- aggregate(mpg ~ cyl + am, FUN = sum, data = mtcars)
attr(df$mpg, "reducer") <- list(summaryfn = sum, initialfn = function() 0)
```

This metadata can be later used for transformations like stacking, at the point when we go on to map the aggregated variables to aesthetic encodings.

```{r}
#| eval: false
summarized_dfs <- compute(
  summaries = list(stat = sum(mpg)), 
  partitions = c(~ cyl, ~ cyl + am),
  data = mtcars
  )

# Computed over the ~ cyl + am product factor
segment_df <- summarized_dfs[2] 

encoded_segment_df <- encode(
  list(
    x = cyl,
    y = stack(stat)
  )
...
Further, for operations like stacking, to ensure that they are applied in correct order, we also need to know the corresponding parent-level partitioning indices. To give a concrete example, here is a simplified implementation in R (see also Section \@ref(aggregation-summaries)):  

```{r}
df <- aggregate(mpg ~ cyl + am, FUN = sum, data = mtcars)
attr(df$mpg, "reducer") <- list(summaryfn = sum, initialfn = function() 0)

stack_reduced <- function(x, parent_indices) {
  reducer <- attr(df$mpg, "reducer")
  result <- vector(class(x))
  stacked <- vector(class(x))
  stacked[1:length(unique(parent_indices))] <- reducer$initialfn()
  
  for (i in seq_along(x)) {
    pidx <- parent_indices[i]
    stacked[pidx] <- reducer$summaryfn(stacked[pidx], x[i])
    result[i] <- stacked[pidx]
  }
  
  result
}

# Stacking within the levels of the cyl variable
parent_indices <- match(df$cyl, unique(df$cyl)) 
df$mpg_stacked <- stack_reduced(df$mpg, parent_indices)
df
```

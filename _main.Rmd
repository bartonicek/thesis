...
Now comes the key point. Suppose that we have some summary function $F: D \to S$ which maps data subsets to summaries. To preserve the structure of $\mathcal{D}$, the operation should be a functor $F: \mathcal{D} \to \mathcal{S}$, which maps data subsets in $D$ to summaries in $S$ and set unions $D_i \cup D_j$ to combinations of summaries $S_i \otimes S_j$, in a way which preserves composition. Specifically, by substituting our morphisms into the composition-preserving property of functors:

$$F(- \cup D_{i} \then - \cup D_{j}) = F(- \cup D_i) \then F(- \cup D_j)$$

The expression above appears somewhat awkward due to the mixing of function expressions with the binary operator $\cup$ and the composition operator $\then$ (i.e. if instead of $- \cup D_i$ and $- \cup D_j$ we had $f$ and $g$, the expression $F(f \then g) = F(f) \then F(g)$ would look much cleaner, even though it expresses the same concept). However, it can be greatly simplified by noting two facts:
...
- We can omit the composition operator $\then$. This is due to the fact that $\cup$ and $\otimes$ are associative binary operators (associativity follows from the definition of composition in categories). As an example, if $f(x) = - \otimes 2 = x + 2$, then we can write $[f \then f](x) = f(f(x)) = ((x + 2) + 2) = x + 2 + 2$)
...
I have argued that, for plots to be well-behaved under certain kinds of interactions such as linked selection, the summary statistics we display should "behave like set union". Specifically, they should be a functor from the preorder of data subsets. This has lead to two key insights: the summary statistic should be based on some binary associative operation $\otimes$, and the summarized values should be equipped with some "neutral" value that is the result of summarizing an empty set. It turns out there is a well-known algebraic structure with these exact properties: a monoid. 
...
Further, even the set union $\cup$ operation forms a monoid, the empty set $\varnothing$ as the neutral/identity element. So, in a certain way, the mapping of data subsets to summaries $F$ can be seen as a mapping between two monoids - a *monoid homomorphism* (which is just an alternative term for a functor between monoids that comes from abstract algebra). However, $F$ is more than just a monoid homomorphism; monoids alone do not capture the hierarchical structure we want. Instead, $F$ is a mapping between two structures (categories) that have the features of both a monoid *and* a preorder. As a sidenote, the technical term for a monoid that is also a preorder is, unsurprisingly, a *monoidal preorder* [@nlab2024e]. Thus, $F$ could technically be called a *"monoidal preorder homomorphism"*, however, it is far easier and clearer to simply refer to it as a functor.
...
Couple of notes. First, based on what we have shown before, we could simplify the problem by defining `fn` to always take two scalar arguments instead of a vector. This would clearly demonstrate that e.g. `mean2 == median2` (median of even number of elements is the average of the middle two), and so if we can show that one function is not associative, the other is neither. Also, more intuitively, with 3 values, summarizing with `median2` amounts to averaging the values pairwise, and clearly this is a different operation from picking the middle value.

Second, technically, the properties of associativity and unitality should hold for *all* possible values, thus testing that a summary function passes the `is_monoid` test for any three specific data vectors is not sufficient to call it a monoid. For instance, it is the case that `mean(c(c(1, 2), 3/2)) == mean(c(mean(c(1, 2)), 3/2))`, however this does not prove that `mean` is associative (in fact, with a little bit of algebra, it can be shown that the equality holds for any triplets $x, y, z$ where $3x + 3y + 6z = 4x + 4y + 4z$). However, a single counter-example does disprove the summary function being monoidal, as is the case, for example, with `mean`, `median`, and `sd` above.
...
3. $h = f \then g$ (read "apply $f$ then $g$")
:::

Throughout this thesis, I will use the bracket notation ($h(x) = g(f(x))$) when explicitly referring to the variable, and the postfix/fat semicolon notation ($h = f \then g$) otherwise. 
...
```{r bijection}
#| echo: false
#| fig-cap: "A bijection does not necessarily have to be composed of bijections. The function $f$ is not surjective, and the function $g$ is not injective, nevertheless, their composition $f \\then g$ yields a bijective function."
...
Like monotone maps, monoid homomorphisms also compose. Proof: suppose $f: X \to Y$ and $g: Y \to Z$ are monoid homomorphisms. Then the composite function $f \then g$ preserves the product:

\begin{aligned}

(f \then g)(x_1 \otimes x_2) & = g(f(x_1 \otimes_X x_2)) \\
& = g(f(x_1) \otimes_Y f(x_2)) \\
& = g(f(x_1)) \otimes_Z g(f(x_2)) \\
& = (f \then g)(x_1) \otimes_Z (f \then g)(x_2)

\end{aligned}

As well as the identity element:

$$(f \then g)(e_X) = g(f(e_X)) = g(e_Y) = e_Z$$
...
- For every three objects $c_1, c_2, c_3 \in \text{Ob}(\mathcal{C})$ and two morphisms $f \in \mathcal{C}(c_1, c_2)$ and $g \in \mathcal{C}(c_2, c_3)$, we define a composite morphism $f \then g \in \mathcal{C}(c_1, c_3)$  

Such that the composition operation is:

1. Unital: $\text{id}_{c_1} \then f = f$, and $f \then \text{id}_{c_2}=f$
2. Associative: $(f \then g) \then h = f \then (g \then h) = f \then g \then h$
...
The properties of a category simply tells us that staying in one place before or after moving to a different place is that same as just moving, $\text{id}_1 \then f = f \then \text{id}_2 = f$, and that how we choose to "lump" the different parts of our journey together does not matter, $(\text{id}_1 \then f) \then \text{id}_2 = \text{id}_1 \then (f \then \text{id}_2)$.
...
2. Associativity: $x \leq y$ and $y \leq z$ $\implies x \leq z$. This is the consequence of associativity of composition in categories: if there is a morphism $x \to y$ and $y \to z$, then we automatically get a morphism $f \then g: x \to z$.
...
2. Preserves composition: for every three objects $c_1, c_2, c_3 \in \text{Ob}(\mathcal{C})$ and two morphisms $f: c_1 \to c_2$ and $g: c_2 \to c_3$, $F(f \then g) = F(f) \then F(g)$

...
Another utility function is `Getter.proxy`, which takes a `Getter` and an array of indices, and returns a new `Getter` which proxies the access to the original values through the indices:
...
In many data analytic workflows, a fundamental data structure is a `Dataframe`. Typically, a `Dataframe` rerpresents a two-dimensional table of columns. In `plotscape`, it is a bit more flexible in that it is just a record of [`Indexable`](#Indexable)s:

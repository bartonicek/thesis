...
That is, a `Reducer<T, U>` has a name, a zero-argument function (or a "thunk") producing a value of type `U`, and a binary function which takes two values of type `T` and produces another `T`. For example, the following are examples of valid `Reducer`s:

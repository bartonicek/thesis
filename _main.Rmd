...
Couple of notes. First, based on what we have shown before, we could simplify the problem by defining `fn` to always take two scalar arguments instead of a vector. This would clearly demonstrate that e.g. `mean2 == median2` (median of even number of elements is the average of the middle two), and so if we can show that one function is not associative, the other is not either. Also, more intuitively, with 3 values, summarizing with `median2` amounts to averaging the values pairwise, and clearly this is a different operation from picking the middle value.

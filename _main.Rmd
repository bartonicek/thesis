...
Yet, a number of unresolved issues surrounding the use and understanding interactive figures remain. While favored by large organization, individual analysts rarely integrate interactive data visualization into their workflows [see e.g. @batch2017]. Further, despite years of research and development into sophisticated interactive features, only a small fraction of them make it into widely available, general software (see Section \@ref(background)). Finally, a core underlying issue that remains to be solved is that of data pipelines: how do we go about modeling process of transforming raw data into something that can be interactively visualized, in a simple and efficient manner [see e.g. @wickham2009; @vanderplas2020]?

In this thesis, I explore these gaps the challenges that come up when developing interactive data visualization systems. A recurring theme will be that, although plots are visual entities composed of geometric objects, they are not mere pictures. Instead, they contain a significant amount of implicit, algebraic structure, which becomes particularly relevant when interaction is introduced. By focusing on the this algebraic foundation, I build upon the work of Leland Wilkinson, and the Grammar of Graphics (GoG) model in particular [-@wilkinson2012]. However, I contend that while GoG offers a foundational framework, to fully encompass the scope of interactive graphics it may require certain refinements. To put it bluntly, consider the following two quotes:

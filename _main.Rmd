...
In this thesis, I argue that the independence model, despite many advantages, is fundamentally inadequate for fully understanding graphics, particularly when interaction comes into play. Visualization components are not independent, but instead deeply interconnected. Throughout this work, I explore this interconnectedness, the challenges is poses for interactive data visualization systems, and propose an alternative model based on some fundamental concepts from category theory. To put these ideas into practice, I also present `plotscaper`, an R package developed to refine these theoretical ideas and offer a practical tool for interactive data exploration. My goal is to demonstrate that, to design truly robust and consistent interactive data visualization systems, we need to ground our thinking in algebraic concepts.

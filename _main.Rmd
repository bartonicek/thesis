...
However, an important issue related to stacking is that the way we visually represent data is *not* independent of the way we summarize it. As discussed in Section \@ref(stackable-or-not), past data visualization researchers have noted the fact that while some summary statistics such as sums or counts can be effectively stacked, others like averages or quantiles cannot [see e.g. @wilke2019; @wills2011; @wu2022]. For instance, while the "sum of sums" or "maximum of maximums" yield valid global statistics, the "average of averages" typically diverges from the grand mean. This discrepancy extends beyond barplots: certain statistics allow for the representation of composite objects, while others do not. Such constraints directly contradict the independence-based approach developed by Grammar of Graphics [@wilkinson2012] and popularized by `ggplot2` [@wickham2016], which assumes that statistical transformations and geometric objects (`geoms`) can be modularly combined at will, without restriction.

Early in the project, I came across some ideas from functional programming, which in turn lead me to the key insight: the part-whole relationships underlying geometric objects and summary statistics can be described algebraically, by using some fundamental concepts from category theory. This lead me to develop a simple algebraic model of graphics, relying on functors, monoids, and groups. By grounding our reasoning in this algebraic model, we can quickly identify which combinations of statistics and geometric objects will permit part-whole relationships, and thus behave well under features like linked selection, and which ones will not. For instance, since the average operator is not a monoid, it prohibit part-whole relations, and thus we know that it will pose a challenge if we try to combine it with linked selection. Further, the model also allows us to identify what *kinds* of linked selection will work. For instance, because the maximum and convex hull operators are monoids, they will work well when comparing nested subsets (e.g. single selection group vs. all cases). However, because they lack an inverse, they will be inappropriate for comparing disjoint subsets (multiple distinct selection groups, see Section \@ref(groups-inverses)). 

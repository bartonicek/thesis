...
```{ts}
type EffectFn = () => void;
const currentEffects = [] as EffectFn[]; // A stack of effects

function createSignal<T>(x: T): [() => T, (value: T) => void] {
  const listeners = new Set<EffectFn>();

  function get(): T {
    // Add the topmost effect on the stack to listeners
    listeners.add(currentEffects[currentEffects.length - 1]);
    return x;
  }

  function set(value: T) {
    x = value;
    // Call listeners, like in Observer
...
function createEffect(effectfn: EffectFn) {
  function call() {
    currentEffects.push(call); // Push effect onto stack
    effectfn();                // Call it, registering listeners
    currentEffects.pop();      // Pop effect off the stack
  }

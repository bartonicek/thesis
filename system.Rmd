...
Every data visualization involves data. In typical column-based data representation such as CSV, data is stored in contiguous arrays. 

Thus, `Indexable` is one of the most fundamental types. It has the following signature:
...
When visualizing data, we often need to split the data into disjoint parts and compute summaries on these parts. The set of all of these parts forms a partition, and, an important fact about partitions in data visualization is that they may be organized in a hierarchy, such that the parts in a child partition "add up" to the parts on the parent partition. A `Factor` provides a way to represent one such partition, as well as the associated metadata. In this way, it is similar to base R's `factor` S3 class, although there are some differences.

A `Factor` has the following interface:
...
The two important properties which define how a `Factor` partitions data are indices and cardinality. Cardinality represents the number of unique indices that the factor has. For example, if the factor represents boolean partition of the data, the cardinality will be 2, if it represents a partition of the data between three states (such as `"A"`, `"B"`, and `"C"`), the cardinality will be 3, if it represents a partition of the data between all countries in the world, the cardinality will be 195, and so on. Indices represent the actual assignment of cases (rows of the data) to the parts. For example, the array of indices `[1, 0, 1, 1, 2]` represents the first, third, and fourth case being assigned to part two, the second case being assigned to part one, and the fifth case being assigned to part three (keeping in mind JavaScript's zero-based indexing).

A factor may have metadata associated with each part, which stored in the `data` property of type [`Dataframe`](#Dataframe). This is a departure from base R's `factor` class, which represents all metadata as the vector of `levels`. For instance:

```{r}
cut(1:10, breaks = c(0, 5, 10))
```

In my system, the same information would be represented as:
...
Constant factors, or `Factor.mono`, are the polar opposite of bijection factors. They assign all cases of the data to a single part (thus, the label function is a constant function, hence the name). The constructor signature is essentially the same as for bijection factor, so I will omit it here.
...
If `breaks` are provided, the constructor function uses those to construct the histogram. Otherwise it uses `nBins`, `width`, and `anchor` to determine the breaks, in the corresponding order of importance.
...
So far, we have discussed scales as a sort of bridge between two properties of type `Expanse` -  the domain and the codomain - however, we have left the precise nature of the `Expanse` type vague. Now it is time to finally time discuss the concrete implementations of the various `Expanse` subtypes.

As discussed previously, the job of the `Expanse<T>` is to translate values in its data domain (`T`) to and from the interval $[0, 1]$. This makes `Expanse` essentially equivalent to the maps discussed in Section \@ref(two-component-scales), with some additional functionality. Thus, each subtype of `Expanse<T>` provides two important functions:

- A *normalize* function $n: T \to [0, 1]$, mapping values from $T$ to $[0, 1]$
- An *unnormalize* function $u: [0, 1] \to T$, mapping values from $[0, 1]$ to $T$

Or, in code:

```{ts}
#| eval: false
function normalize<T>(expanse: Expanse<T>, value: T): number
function unnnormalize<T>(expanse: Expanse<T>, value: number): T
```

To implement their behavior, the functions rely on properties that are specific to each `Expanse` subtype. Besides the two functions, each subtype of the `Expanse` type also need to implement few additional functions, such as the `train` function which allows the `Expanse` to be trained on new data and the `breaks` functions which returns a list of breaks (an array of type `T[]`).  

#### Continuous expanses

A continuous expanse is a generalization of the linear mapping discussed in Section \@ref(two-component-scales). That is, it translates values to and from a continuous interval given (roughly) by $[\text{min}, \text{max}]$. To do this, it uses the following properties:
...
The `min` and `max` properties are fairly self-explanatory. The `offset` property allows us to move values by some constant, either before they have been normalized or after they have been unnormalized. This is useful, for example, when we want to ensure that the width of a spineplot bar is 1 pixel less than the available space. The `trans` and `inv` properties allow us to perform non-linear transformations (they should, intuitively, be inverses of each other). Finally, the `ratio` property is a simple boolean flag which indicates whether the expanse is part of a ratio scale. If this flag is set to `true`, this indicates that the `min` value of the expanse must always be zero and we cannot change it by, for example, training on new data.

...
#### Scales and expanses {#scales-expanses}

As discussed in Section \@ref(scaling), to actually visualize data, we need to be able to translate values from its domain to the domain of the graphical device (computer screen). In many data visualization packages, this work is done by specialized components called scales. The theoretical model of scales `plotscape` uses was already discussed in Section \@ref(scaling), with the key takeway being that a scales are fundamentally composed of two components: its domain and codomain. In Section \@ref(scales-comparison), I briefly discussed the fact that many popular data visualization systems treat the domain and codomain as fundamentally different entities. I argued that this creates unnecessary complexity, and instead suggested a unified model where both the domain and codomain are represented by the same types. In this section, I will discuss the `plotscaper`'s implementation of scales, `Scale`, as well as the implementation of the domain and codomain components - `Expanse`.  

##### Scales {#scales}

As discussed in Section \@ref(scaling), a key idea for creating a generic model of scales is to imagine the scale as being composed of two components, each translating values to and from their respective domain and the real numbers $\mathbb{R}$. That is, first assume we have some generic type `Expanse<T>` and a corresponding namespace which exposes two functions:
...
`Expanse` will be discussed in greater detail later in Section \@ref(expanses), however, for now, look simply at the type signatures of `Expanse.normalize` and `Expanse.unnormalize` - these functions do what we want, in that they translate values of type `T` to number and vice versa. Assuming that `Expanse` really behaves this way, we can define the `Scale` interface as follows:
...
The two primary properties of `Scale` are the `domain` and `codomain`, both of type `Expanse`. The utility of the other several numeric properties was briefly mentioned in Section \@ref(scaling) and will be discussed later, however, for now, we can ignore them. With only the `domain` and `codomain`, we can already implement a simplified version of the main functionality of `Scale` - the `pushforward` and `pullback` functions:
...
As you can see, `Scale.pushforward` takes as its arguments a `Scale<T, U>` and a value of the same type as the `Scale`'s `domain` (`T`). It then normalizes this value to a `number`, using the `domain`, finally turns the number back to the value of codomain's type (`V`) by using the `unnormalize` function.

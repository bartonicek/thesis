...
Several practical consideration have lead to choosing web-technologies as the underlying platform as well as splitting the functionality across two packages. First, web-technologies were chosen is because they provide a simple portable way to do interactive apps in R, having become the de facto standard thanks to good integration, due to packages such as `htmlwidgets` [@htmlwidgets2021] and Shiny [@sievert2020]. Further, to achieve performance and fine-grained control, `plotscape` was implemented independently, in vanilla TypeScript/JavaScript, rather than with some JS-in-R adapter library. The `plotscaper` package was then developed as a high-level wrapper that would make the `plotscape` more accessible and convenient for R users.  

As of writing, `plotscape` comprises of about 6,400 significant lines of code (SLOC; un-minified, primarily TypeScript but also some CSS, includes tests, etc...), whereas the `plotscaper` contains about 500 additional SLOC of R code (both counted using [`cloc`](https://github.com/AlDanial/cloc)). The unpacked size of all files is about 200 kilobytes for `plotscape` and 460 kilobytes for `plotscaper`, fairly modest size in comparison to other interactive data visualization packages (for instance, the `plotly` package takes up about 7.3 megabytes which amounts to over 15x difference).

Since the two packages address fairly well-separable concerns - high-level API design vs. low-level interactive visualization utilites - I organize this section accordingly, by discussing high-level concerns alongside `plotscaper` first and low-level details alongside `plotscape` second. Cross-cutting concerns will be addressed towards ends of the respective sections. However, first, let's briefly review the core requirement of the package(s).

## Core requirements

To re-iterate, from Section \@ref(goals), the main requirements for the high-level API (`plotscaper`) were:
...
However, first, it was also necessary to design the low-level platform (`plotscape`) on top of which these goals could be realized. The primary purpose of `plotscape` was to provide utilities for:

- Splitting the data into a hierarchy of partitions
- Computing and transforming summary statistics (e.g. stacking, normalizing), while respecting the hierarchy
...
- Responding to user input and propagating changes throughout the entire pipeline

## High-level API (`plotscaper`)

### API design

As mentioned in Section \@ref(goals), a primary inspiration for `plotscaper`'s API was the popular R package `ggplot2`. In `ggplot2`, plots are created by chaining together a series of function calls, each of serves to compose an (immutable) plot schema:
...
However, while it is the case that the `ggplot2` well-loved by R users, its API has some disadvantages when it come to building interactive figures. Specifically:

- Its design primarily focuses on individual plots. While facetting does make it possible to create multi-panel figures consisting of repeats of the same plot type [see `facet_wrap()` and `facet_grid()`, @wickham2016], multi-panel figures with mixed plot types require external packages such as `gridExtra` [@auguie2017] or `patchwork` [@pedersen2024]. As discussed in Section \@ref(background), in interactive graphics, multi-panel figures composed of different plot types are essential.
- While the immutable schema model works well for static graphics, in interactive graphics, the ability to modify an already rendered figure can be extremely useful. For example, by directly mutating the figure state via a function call, we can easily set an interactive histogram binwidth to a precise value rather than having to rely on an imprecise widget such as a slider.   
- Many of the `ggplot2`'s core functions rely on non-standard evaluation [NSE, @wickham2019]. While NSE is fairly popular within the R ecosystem and does offer syntactic conciseness, it also hinders programmatic use [@wickham2019]. For instance, in `ggplot2`, to plot all pairwise combinations of the variables in the data, we cannot simply loop over their names and supply these as arguments to the default `aes` function - instead, we have to use the specialized `aes_string` function which accepts string arguments. Again, in interactive contexts, the ability to easily manipulate figures with code is often highly desirable, and this makes NSE a hindrance.  

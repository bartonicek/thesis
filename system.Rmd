...
As discussed in the previous section, each `Scale` consists of two properties of type `Expanse`, which represent the space the values are mapped *from* - the `domain` - and the space the values are mapped *to* - the `codomain`. Thus, a `Scale` mainly acts as a sort of a connector between the `domain` and `codomain`, with most of the real work being done by these two properties.

...
When it comes to encapsulation in DOP, we have to differentiate between encapsulating data and encapsulating code. Encapsulating code is easy in DOP - we can simply not export certain functions from the code modules. We are then free to modify the signature of these functions without affecting the public interface [@fabian2018]. Encapsulating data may be a bit more work. Depending on the language we use, generic data structures may not come equipped with property access modifiers [although there does seem to be a trend in recent languages to support property access modifiers more generically, see e.g. @rust2024; @zig2024]. For example, in JavaScript, private properties can only be declared as part of a class declaration [@mdn2024e]. However, this does not necessarily mean that encapsulation of data is impossible - in most languages, there will most likely exist some way to emulate private property access, e.g. by using metaprogammming.

foo bar baz

For example, here's how we can emulate private property access in JavaScript using Proxy:

```{ts}
// Make object properties modifiable only from within the namespace
function makePrivate<T extends Object>(object: T, namespace: Object) {
...
namespace User {
  ...
  export function of(firstName: string, lastName: string): User {
    return makePrivate({firstName, lastName}, User);
  }
  ...
  // We could do the same thing for function set() { ... }

  export function getFullName(user: User) {
    // Uses the internal get() function we defined above
...
In the code above, we managed to encapsulate data even while maintaining a clear separation between data and code. Specifically, `User` is still just a generic object and can be inspected via `console.log`, however, we cannot access or modify its properties outside of the `User` code module.

Thus, encapsulation of data is certainly possible in DOP. However, a question still remains whether it is a good idea. While encapsulation is generally seen as a net positive in OOP, in the DOP view, data encapsulation presents trade-offs. Whereas it might provide a layer of security, it also makes systems more complex, harder to debug, and even with encapsulated data users may still come to rely on hidden features of the system [@fabian2018; @sharvit2022]. Thus, it is necessary to weight the pros and cons of encapsulation.

In JavaScript/TypeScript, it is also possible to 

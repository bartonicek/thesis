...
::: {.definition #simple name="Simple scale function"}
...
It is relatively simple to translate the formula in \@ref(def:simple) to code:
...
#### The limits of modeling scales as simple functions
...
::: {.definition name="Two-component scale system"}

$$s(d) = \color{steelblue}{v_{min} +} \color{indianred}{\frac{d - d_{min}}{d_{max} - d_{min}}} \color{steelblue}{\cdot (v_{max} - v_{min})}$$
...
console.log(unnormalize(normalize(5.5, 1, 10), 0, 800))
```

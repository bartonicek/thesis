...
As you can see, zooming into the middle 50% of a scale that already includes margins has a smaller effect on `zero` and `one`, since the margins have effectively expanded the space we're zooming into (i.e., a scale with margins is already *zoomed out*, in a way).

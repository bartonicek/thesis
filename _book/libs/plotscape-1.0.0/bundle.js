var PLOTSCAPE = (() => {
    ...
    define("functions", ["require", "exports"], function (require, exports) {
        ...
        const prettyBreaks = (x, n = 4) => {
            ...
            const neatValues = [1, 2, 4, 5, 6, 8, 10];
            const dists = neatValues.map((e) => Math.pow((e - unitGross / Math.pow(10, base)), 2));
            const unitNeat = Math.pow(10, base) * neatValues[dists.indexOf(Math.min(...dists))];
            const big = Math.abs(base) > 4;
            const minNeat = Math.round(min / unitNeat) * unitNeat;
            const maxNeat = Math.round(max / unitNeat) * unitNeat;
            const middle = Array.from(Array(Math.round((maxNeat - minNeat) / unitNeat - 1)), (_, i) => minNeat + (i + 1) * unitNeat);
    ...
    define("plot/Plot", ["require", "exports", "functions", "auxiliaries/auxiliaries", "handlers/handlers", "plot/GraphicStack"], function (require, exports, funs, auxs, hndl, GraphicStack_js_1) {
        ...
        class Plot extends GraphicStack_js_1.GraphicStack {
            constructor(plotConfig) {
                const { id, element, data, mapping, globals } = plotConfig;
                super(element);
                this.resize = () => {
                    const graphicLayers = ["graphicBase", "graphicUser", "graphicHighlight"];
                ...
                this.initialize = () => {
                    const { handlers, scales, callChildren, onMouseDownHere, onMouseDownAnywhere, onDoubleClick, onKeypress, onKeyRelease, drawBase, drawHighlight, drawUser, startDrag, whileDrag, endDrag, containerDiv, sceneDiv, } = this;
                    ...
                    sceneDiv.addEventListener("dblclick", onDoubleClick);
                    sceneDiv.addEventListener("mousedown", onMouseDownAnywhere);
                    containerDiv.addEventListener("mousedown", onMouseDownHere);

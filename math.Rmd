...
Additionally, the monoidal product $\otimes$ has an inverse^[Note, that in group theory, it is common to omit explicit references to the operator and write the group products without it, such that, e.g. $m \otimes n$ is written as $mn$. In that case, instead of the inverse *operator*, we speak of inverse *elements*, such that, for each $m \in M$ there is an $m^{-1}$ such that $m^{-1}m = mm^{-1} = e$. However, this really is a distinction without difference since we can easily define the inverse element using the inverse operator and the neutral element: $m^{-1} = (e \otimes^{-1}m)$. Thus, for the sake of keeping with the notation in the previous sections, I will use the inverse operator explicitly.] $\otimes^{-1}$, such that the following property holds:

3. Invertibility: $m \otimes^{-1} m = e$, for all $m \in M$
:::

Invertibility implies that, when we take a product of any two elements, we can always recover either one by "subtracting it away" via the inverse product with the other.
...
It might not seem as if this broad, abstract definition buys us much. However, as we will see, it allows us to reason about all of the algebraic structures we have discussed so far. For example:

:::{.proposition}
A preorder is a category where there is at most one morphism between any two objects.
:::

We discussed preorders as tuples of a set and a relation, $(X, \leq)$, which fulfills two fundamental properties: reflexivity and associatvity. If we cast preorder as a category, these properties automatically fall out of the definition. Specifically, let the elements $x \in X$ be the objects in the category $\mathcal{X}$ corresponding to the preorder $(X, \leq)$, and let the relation $leq$ be represented by presence of a morphism between two objects (of which can there be only one). 
...
2. Associativity: $x \leq y$ and $y \leq z$ $\implies x \leq z$. This is the consequence of associativity of composition in categories: if there is a morphism $x \to y$ and $y \to z$, then we automaticlly get a morphism $f \then g: x \to z$.

But what do we mean when we say that a preorder "is" a category? 

The example with preorder was fairly intuitive, since the elements $x \in X$ mapped to objects and the relation $\leq$ mapped to morphisms. However, this is not the only way that we can recast algebraic objects as categories. For example:

::: {.proposition}
A monoid is a category with a single object.
:::

Here, we need to break away from the intuitive way of thinking of objects as elements in a set and morphisms as functions, and instead take a more creative approach.
